...
public class SetupEvalBean {
	...
	*/
	...
	public String saveAndAssignEmailTemplate() {
		...
		evaluationSetupService.assignEmailTemplate(emailTemplate.getId(),
				evaluationId, null, commonLogic.getCurrentUserId());

		// user message
		EvalEvaluation eval = evaluationService.getEvaluationById(evaluationId);
		messages.addMessage(new TargettedMessage(
				"controlemailtemplates.template.assigned.message",
				new Object[] { emailTemplate.getType(),
						emailTemplate.getSubject(), eval.getTitle() },
	 ...
	 */
	public static String[] makeArrayFromBooleanMap(
			Map<String, Boolean> booleanSelectionMap) {
		...
		String[] selectedHierarchyNodeIds = hierNodeIdList
				.toArray(new String[] {});
